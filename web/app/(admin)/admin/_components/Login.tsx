...
// Admin phone-OTP login (doc 03 §5: login is phone-OTP for every role). Same two
// steps and same /auth endpoints as the coordinator console; the seeded admin is
// Priya Sharma (+915550000001). Locally OTP_DEBUG_ECHO returns the code, surfaced
// as a hint so the demo needs no SMS provider.

import { useState } from "react";
import { requestOtp, verifyOtp } from "@/app/_lib/queue";

export function Login({ onToken }: { onToken: (token: string) => void }) {
  const [phone, setPhone] = useState("+915550000001"); // seeded admin
  ...
  return (
    <main className="login">
      ...
      <section className="card">
        ...
        <h1>Sign in</h1>
        <p className="sub">Edit content and watch the numbers. Admin accounts only.</p>
