...
// Phone-OTP login for the doctor (doc 03 §5: phone OTP for everyone). Lifted
// from the S8 coordinator login, which was written to be reused here — same two
// steps, same /auth endpoints, different copy and a doctor default.

import { useState } from "react";
import { requestOtp, verifyOtp } from "@/app/_lib/queue";

export function Login({ onToken }: { onToken: (token: string) => void }) {
  const [phone, setPhone] = useState("+915550001001"); // seeded Dr. Anil Gupta
  ...
  return (
    <main className="login">
      ...
      <section className="card">
        ...
        <h1>Sign in</h1>
        <p className="sub">Your morning list, and every patient&rsquo;s story in twenty seconds.</p>
...
.login .badge { display: inline-block; font-size: 12px; font-weight: 700; letter-spacing: .08em;
  text-transform: uppercase; color: var(--primary-d); background: var(--primary-soft);
